...
def generate_usernames(mod_list):
   ...
   """
   ### WRITE SOLUTION CODE HERE
   def replace_with_underscore(str):
      return str.replace(' ', '_')
   
   mod_list = [replace_with_underscore(user_name) for user_name in mod_list]
   return mod_list
#    raise NotImplementedError()
...
def map_id_to_initial(employee_list):
   ...
   """
   ### WRITE SOLUTION CODE HERE
   def emp_initial(employee_list):
      return employee_list['name'][0]
   def emp_id(employee_list):
      return employee_list['id']
   employee_initial_list = [emp_initial(employee) for employee in employee_list]
   employee_id_list = [emp_id(employee) for employee in employee_list]
   map_id_to_initial = { key:value for (key, value) in zip(employee_initial_list, employee_id_list) }
